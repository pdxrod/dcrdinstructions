...
`./dcrdata --pg --testnet --dcrduser=rpcuser --dcrdpass=rpcpass --dcrdserv=localhost:19108`

(The last three flags depend on your RPC username and password for the Decred daemon, and which port it is serving on. See dcrd section above).
...
`./dcrdata --pg --testnet` to start dcrdata.
